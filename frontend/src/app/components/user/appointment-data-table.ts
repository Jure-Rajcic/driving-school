...
@Component({
  ...
  providers: [
    provideIcons({ lucideChevronDown, lucideCopy, lucideArrowUpDown }),
  ],
...
export class AppointmentDataTableComponent {
  @Input() set data(newData: AppointmentDTO[]) {
    ...
    selectedAppointments.forEach((selectedAppointment) => {
      const appointment = this._data().find(
        (appointment) =>
          JSON.stringify(appointment) === JSON.stringify(selectedAppointment)
      );
      if (appointment) {
  ...
  protected readonly _isAppointmentSelected = (row: AppointmentDTO) =>
  ...
  @Output()
  protected readonly appointmentsRequested = new EventEmitter<
    AppointmentDTO[]
  >();
  protected onAppointmentsRequested($event: AppointmentDTO[]) {
    this.appointmentsRequested.emit($event);
  }
}
